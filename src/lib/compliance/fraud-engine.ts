// src/lib/compliance/fraud-engine.ts
// Fraud Detection Engine — risk scoring, signal detection, rule evaluation
...
export interface ScoreTransactionRequest {
  ...
  customerCreatedAt?: string; // ISO timestamp
  ipAddress?: string;
}
...
async function checkUnusualAmount(
  ...
  if (!data || data.length < 5) return []; // not enough history

  // Flag very large amounts as unusual
  if (amount > 200000) {
    return [
      signal(
        'unusual_amount',
...
// Safe rule evaluator using a restricted comparison engine — no eval/Function.
// ─────────────────────────────────────────────────────────
...
/**
 * Evaluates a single comparison expression of the form:
 *   <identifier> <operator> <number>
 * Supported operators: >, <, >=, <=, ==, !=
 * Compound expressions (AND/OR) are split before calling this.
 */
...
function evaluateRule(rule: FraudRule, request: ScoreTransactionRequest): boolean {
  ...
  // Handle OR — at least one part must be true
  ...
  for (const row of rules) {
    ...
    if (evaluateRule(rule, request)) {
      ...
      if (rule.scoreImpact > 0) {
        signals.push(
          signal(
            'card_testing', // generic "custom rule triggered" signal
...
export async function scoreTransaction(
  ...
  const [velocitySignals, unusualSignals, { signals: ruleSignals, forcedAction }] =
    await Promise.all([
      checkVelocity(request.customerId, request.ventureId, request.amount),
      checkUnusualAmount(request.customerId, request.ventureId, request.amount),
...
export async function listFraudRules(ventureId: string): Promise<FraudRule[]> {
  if (!supabase) return [];
  const { data } = await supabase
    .from('fraud_rules')
    .select('*')
