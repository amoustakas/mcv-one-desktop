// src/lib/commerce/discount-engine.ts
// Discount & Promotion Engine — Commerce Surface Layer
// Handles code validation, auto-discounts, BXGY, stacking, usage tracking
...
import type { CartItem } from './surface-types';
...
export async function getDiscount(id: string, ventureId: string): Promise<Discount | null> {
...
export async function disableDiscount(id: string, ventureId: string): Promise<Discount> {
  return updateDiscount(id, ventureId, { status: 'disabled' });
}
...
export async function validateDiscount(
  ...
  const discount = await getDiscountByCode(ventureId, code);
  ...
  if (discount.minimumQuantity != null) {
    ...
    if (totalQty < discount.minimumQuantity) {
      return {
        valid: false,
        reason: `Minimum quantity of ${discount.minimumQuantity} items required`,
      };
    }
  }
  ...
  if (discount.appliesTo === 'specific_customers' && discount.customerGroupIds.length > 0) {
    ...
    // Check if customer belongs to an allowed group
...
export function calculateBXGY(
  cartItems: CartItem[],
  discount: Discount,
): BXGYResult {
  if (discount.type !== 'bxgy') {
    return { freeItems: [], totalDiscount: 0 };
  }
  ...
  // Find qualifying "buy" items — any product unless specific products set
...
// ─────────────────────────────────────────────────────────
// APPLY DISCOUNTS (cart-level resolution)
// ─────────────────────────────────────────────────────────
...
export async function applyDiscounts(
  ...
  // 1. Collect auto-discounts (no code, active, applies to cart products)
  ...
  // 2. Collect code-based discounts
  const codeDiscounts: Discount[] = [];
  for (const code of discountCodes) {
    const result = await validateDiscount(ventureId, code, cartTotal, customerId, productIds);
    if (result.valid && result.discount) {
      codeDiscounts.push(result.discount);
    }
  }
  ...
  // 3. Stackability: if any non-stackable, pick the best single discount
  const hasNonStackable = allEligible.some((d) => !d.stackable);

  let toApply: Discount[];
  if (hasNonStackable) {
    // Pick single best discount by estimated savings
    ...
    // All stackable — sort: percentage first, then fixed
  ...
  // 4. Calculate per-item discounts
  ...
  for (const discount of toApply) {
    if (discount.type === 'bxgy') {
      ...
      // Map free items back to cart items
      ...
      // Handled at shipping level — no item-level discount
      totalDiscount += 0; // shipping amount resolved elsewhere
      ...
      if (matchedTier) {
        ...
        // Distribute proportionally across applicable items
...
function estimateDiscountSavings(
  ...
  switch (discount.type) {
    ...
    case 'bxgy': {
      const { totalDiscount } = calculateBXGY(cartItems, discount);
    ...
    case 'free_shipping':
      return 0; // unknown shipping at this point
    default:
      return 0;
  }
}
