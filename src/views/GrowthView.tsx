...
import { TrendingUp, Target, Megaphone, BarChart3, Plus, Trash2, Calendar, DollarSign, Edit3, X, Users, Zap } from 'lucide-react';
import { useNavigation } from '../stores/navigation';
import { ventures } from '../lib/ventures';
import { useCampaigns, useCreateCampaign, useUpdateCampaign, useDeleteCampaign } from '../hooks/use-campaigns';
import { PageHeader, PageShell, Button, StatCard, GlassCard, GridLayout } from '../components/ui';
...
export default function GrowthView() {
  ...
  return (
    <PageShell scroll>
      <PageHeader icon={<TrendingUp size={20} />} title="Growth Studio" loading={isLoading} onRefresh={() => refetch()}>
        <Button variant="secondary" size="sm" icon={<Plus size={13} />} onClick={() => setShowAdd(!showAdd)}>Campaign</Button>
      </PageHeader>

      {/* KPI Strip */}
      <GridLayout cols={3} gap="sm">
        <StatCard icon={<Megaphone size={14} />} label="Campaigns" value={campaigns.length} />
        <StatCard icon={<Zap size={14} />} label="Active" value={activeCampaigns} />
        <StatCard icon={<DollarSign size={14} />} label="Total Budget" value={formatMoney(totalBudget)} />
        <StatCard icon={<Users size={14} />} label="Total Reach" value={formatNum(totalReach)} />
        <StatCard icon={<Target size={14} />} label="Conversions" value={formatNum(totalConversions)} />
      </GridLayout>

      {/* Add Form */}
      {showAdd && (
        <GlassCard className="growth-form">
                ...
                <div key={c.id} className="growth-campaign">
                  ...
                  {isEditing ? (
                    <div className="growth-camp-edit">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => setEditId(null)}><X size={11} /></Button>
                    </div>
                  ) : (
                    <div className="growth-camp-actions">
                      <Button variant="ghost" size="sm" onClick={() => setEditId(c.id)}><Edit3 size={11} /></Button>
                      <Button variant="danger" size="sm" onClick={() => handleDelete(c.id)}><Trash2 size={11} /></Button>
                    </div>
                  )}
                </div>
              );
      ...
      <style>{`
        .growth-form { display:flex; gap:6px; padding:10px; align-items:center; flex-wrap:wrap; margin:0 20px; }
        ...
        .growth-grid { display:grid; grid-template-columns:280px 1fr 1fr; gap:16px; padding:0 20px 20px; }
