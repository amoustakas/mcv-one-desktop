// @mcv/compliance-sdk/dunning — Dunning / payment-retry orchestration factory.
//
// Smart retry schedule (day 1/3/5/7 + grace period). Caller injects Supabase;
// null tolerated for offline/test mode.
...
export interface DunningEngine {
  getDunningConfig(ventureId: string): Promise<DunningConfig>;
  updateDunningConfig(
    ventureId: string,
  ...
  processRetries(ventureId: string): Promise<RetryResult[]>;
  markRecovered(dunningId: string): Promise<void>;
  markExhausted(dunningId: string, config?: DunningConfig): Promise<void>;
  getDunningStats(ventureId: string): Promise<DunningStats>;
}
...
export function createDunningEngine({
  supabase,
}: {
  supabase: SupabaseClient | null;
}): DunningEngine {
  async function getConfig(ventureId: string): Promise<DunningConfig> {
  ...
  async function markRecovered(dunningId: string): Promise<void> {
    if (!supabase) return;
    await supabase
      .from('dunning_states')
      .update({
  ...
  async function markExhausted(dunningId: string, config?: DunningConfig): Promise<void> {
    if (!supabase) return;
    await supabase
      .from('dunning_states')
      .update({
  ...
  return {
    getDunningConfig: getConfig,

    async updateDunningConfig(ventureId, config) {
    ...
    async initiateDunning(subscriptionId, paymentIntentId, ventureId) {
      if (!supabase) return null;

      const config = await getConfig(ventureId);
    ...
    async processRetries(ventureId) {
      if (!supabase) return [];
    ...
    async getDunningStats(_ventureId) {
      if (!supabase) {
      ...
      return {
        ...
        exhaustedThisMonth,
        recoveryRate,
        revenueAtRisk: activeCount * 2999,
      };
    },
  };
}
