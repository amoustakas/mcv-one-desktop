// @mcv/storage-sdk/types

// `icon` field on StorageProvider was previously typed as LucideIcon (UI
// concern). The SDK keeps it as `unknown` so callers can attach any icon
// representation without forcing a UI library dep on the SDK.
type LucideIcon = unknown;

/** Runtime context injected into provider methods. Structurally identical
 *  to `@mcv/kits-sdk`'s `KitExecutionContext`; declared locally to keep
 *  storage-sdk free of a circular dep on kits-sdk. TS structural typing
 *  makes the two interchangeable at any call site. */
