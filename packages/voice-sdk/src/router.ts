// @mcv/voice-sdk — Dual-stack voice router.
//
// Picks between:
...
//
// Pure selection logic. Zero runtime dependencies — every MCV venture app
// consumes this same router, so voice routing decisions are consistent
// across MCV Desktop, FutureState, BetEdge, mcv.gg, WarForge, etc.
...
export type VoiceUseCase =
  ...
  | 'multi-agent-roundtable'    // multiple agents in one session (Gemini Live)
  ...
  | 'claude-chat-voice'         // Claude-primary chat gets voice via D+E
  | 'auto';                     // router decides from context

export interface VoiceContext {
  useCase: VoiceUseCase;
  /** LLM driving the current session — 'google' routes to Live natively */
...
/**
 * Agent → voice persona profile.
 * MCV Desktop overrides resolvePersonaVoice with a Supabase-backed lookup;
 * this is the default shipped with the SDK for any new venture.
 */
...
export const DEFAULT_PERSONA_VOICES: Record<string, PersonaVoice> = {
...
/**
 * Default persona resolver — looks up DEFAULT_PERSONA_VOICES with no DB.
 * MCV Desktop's src/lib/voice/router.ts wraps this with a Supabase lookup
 * that falls through to these defaults when no custom mapping exists.
 */
export async function resolvePersonaVoice(
  agentCodename: string,
  _ventureId?: string,
): Promise<PersonaVoice | null> {
  return DEFAULT_PERSONA_VOICES[agentCodename.toLowerCase()] ?? null;
}
