// @mcv/commerce-sdk/loan-engine — loan lifecycle + amortization + double-
// entry ledger integration.
//
// 8 methods behind createLoanEngine({ supabase, ledger }) plus
// calculateAmortization exported top-level (pure, used for schedule
// previews without touching the DB).
//
// Lifecycle:
//   application → approved → disbursed → repaying → paid_off
//   any of { disbursed, repaying } can transition → defaulted (write-off)
//
// Ledger entries:
//   disburseLoan     DR 1070 Loans Receivable / CR 1010 Cash
//   recordRepayment  DR 1010 Cash / CR 1070 Loans Receivable (principal)
//                    + CR 4080 Revenue - Interest (interest portion)
//                    Interest is capped by the payment amount when the
//                    accrual would otherwise exceed it — no phantom
//                    interest beyond what's paid.
//   defaultLoan      DR 5060 Loan Write-offs / CR 1070 Loans Receivable
...
/**
 * Calculate full amortization schedule. BNPL (rate=0) uses equal-principal
 * installments; interest-bearing uses the standard
 *   P · (r/12) / (1 − (1 + r/12)^(−n))
...
export interface LoanEngine {
  createLoan(input: CreateLoanInput): Promise<Loan>;
  approveLoan(id: string, ventureId: string): Promise<Loan>;
  disburseLoan(id: string, ventureId: string): Promise<Loan>;
  recordRepayment(loanId: string, ventureId: string, amount: number): Promise<Loan>;
  defaultLoan(id: string, ventureId: string): Promise<Loan>;
  getLoan(id: string, ventureId: string): Promise<Loan | null>;
  listLoans(
    ventureId: string,
    filters?: ListLoansFilters,
  ): Promise<{ loans: Loan[]; total: number; page: number; pageSize: number }>;
  getRepaymentSchedule(loanId: string): Promise<AmortizationRow[]>;
}

export interface LoanEngineOptions {
...
export function createLoanEngine({ supabase, ledger }: LoanEngineOptions): LoanEngine {
  ...
  const engine: LoanEngine = {
    ...
    async approveLoan(id, ventureId) {
      if (!supabase) throw new Error('Supabase client not available');
    ...
    async disburseLoan(id, ventureId) {
      if (!supabase) throw new Error('Supabase client not available');
      ...
      // Disbursement: DR 1070 Loans Receivable / CR 1010 Cash.
    ...
    async recordRepayment(loanId, ventureId, amount) {
      if (!supabase) throw new Error('Supabase client not available');
      ...
      // Split the payment into principal and interest portions. Interest
      // accrual is capped by the payment amount so we don't book phantom
      // interest revenue beyond what was actually paid.
      ...
      // Repayment journal: DR 1010 Cash (total) / CR 1070 Loans Receivable
      // (principal) + CR 4080 Revenue - Interest (interest, if any).
      const cashAccountId = await requireAccount(ventureId, '1010');
      const loansReceivableId = await requireAccount(ventureId, '1070');

      const lines: LedgerJournalEntryInput['lines'] = [
        {
          accountId: cashAccountId,
          debitAmount: amount,
          creditAmount: 0,
    ...
    async listLoans(ventureId, filters) {
      ...
      // Hydrate repayments in a single follow-up query.
    ...
    async defaultLoan(id, ventureId) {
      if (!supabase) throw new Error('Supabase client not available');
      ...
      // Write-off: DR 5060 Loan Write-offs / CR 1070 Loans Receivable.
