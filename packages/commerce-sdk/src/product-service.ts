// @mcv/commerce-sdk/product-service — product CRUD + listing + search.
//
// 6 methods behind createProductService({ supabase }). Products are
// polymorphic via the ProductType enum — one row backs any of the 17
// type-specific configs (physical, digital, subscription, metered,
// credit, loan, investment, gift, transfer). mapProductRow hydrates all
// configs even when null so consumers always see a complete Product.
//
// Note: `searchProducts` here returns Product[] (raw CRUD search). The
// richer faceted SearchResult shape lives in @mcv/commerce-sdk/search-
// engine. The two coexist by design — this is the direct-DB path,
// search-engine is the user-facing analytics-instrumented one.
...
export interface ProductService {
  ...
  getProduct(id: string, ventureId: string): Promise<Product | null>;
  listProducts(ventureId: string, filters?: ListProductsFilters): Promise<Product[]>;
  archiveProduct(id: string, ventureId: string): Promise<Product>;
...
export function createProductService({ supabase }: ProductServiceOptions): ProductService {
  return {
    ...
    async updateProduct(id, ventureId, updates) {
      if (!supabase) throw new Error('Supabase client not available');

      // Build partial update row — only include explicitly provided keys so
      // callers can set columns to null without nuking the rest.
    ...
    async listProducts(ventureId, filters) {
      if (!supabase) return [];
    ...
    async archiveProduct(id, ventureId) {
      if (!supabase) throw new Error('Supabase client not available');
    ...
    async searchProducts(ventureId, query) {
      if (!supabase) return [];
