...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    switch (action) {
      ...
      // ═══════════════════════════════════════
      // Sync Gmail → Documents + CRM
      // ═══════════════════════════════════════
      case 'sync-emails': {
        let token: string;
        try { token = (await getProviderToken(userId, 'google')).token; } catch { return res.json({ synced: 0, skipped: 0, error: 'Google not connected' }); }
        ...
        if (emails.length === 0) return res.json({ synced: 0, skipped: messageIds.length });
      ...
      // ═══════════════════════════════════════
      // Sync Twilio Calls → CRM Activities
      // ═══════════════════════════════════════
      case 'sync-calls': {
        if (!ACCOUNT_SID || !AUTH_TOKEN) return res.json({ synced: 0, error: 'Twilio not configured' });
      ...
      // ═══════════════════════════════════════
      // Sync Calendar → Tasks + Meeting Notes
      // ═══════════════════════════════════════
      case 'sync-calendar': {
        let token: string;
        try { token = (await getProviderToken(userId, 'google')).token; } catch { return res.json({ synced: 0, error: 'Google not connected' }); }
...
`;
      ...
      // ═══════════════════════════════════════
      // Sync Slack/Discord → Documents + Contacts
      // ═══════════════════════════════════════
      case 'sync-messaging': {
        // Slack: fetch messages mentioning user from top channels
        let slackSynced = 0;
        try {
        ...
        results.messaging = slackSynced;
        return res.json({ synced: results, total: slackSynced });
      }

      // ═══════════════════════════════════════
      // Sync Social → Engagement Digest
      // ═══════════════════════════════════════
      case 'sync-social': {
        const today = new Date().toISOString().split('T')[0];
        const sourceId = `social-digest-${today}`;
        if (await isDuplicate('documents', sourceId)) return res.json({ synced: 0, skipped: 1 });
      ...
      case 'full-sync': {
        ...
        for (const syncAction of ['sync-emails', 'sync-calls', 'sync-calendar', 'sync-messaging', 'sync-social']) {
          try {
            // Recursive call with modified body
            const subReq = { ...req, body: { ...req.body, action: syncAction } } as VercelRequest;
            const subResult = await new Promise<Record<string, number>>((resolve) => {
              const fakeRes = {
                json: (data: { synced?: Record<string, number> | number; total?: number }) => {
                  if (typeof data.synced === 'object') resolve(data.synced);
                  else resolve({ total: data.total || 0 });
                },
                status: () => ({ json: () => resolve({ error: 1 }) }),
              };
              handler(subReq, fakeRes as unknown as VercelResponse);
            });
            allResults[syncAction] = subResult;
          } catch {
            allResults[syncAction] = { error: 1 };
          }
        }
